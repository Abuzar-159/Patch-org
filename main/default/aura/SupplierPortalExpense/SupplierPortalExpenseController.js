({
    ...
    doInit : function(component, event, helper) {
        ...
        helper.getExpense(component,event);
    },
    ...
    searchUserRecord : function(cmp,event,helper){
        try{      
            ...
            if(SearchString!='' && SearchString!=undefined){   
                ...
                var bankStmtlist =[]; bankStmtlist=cmp.get("v.ExpenseListDup");
                bankStmtlist = bankStmtlist.filter(function (el) {    
                    return ((el.Name).toString().toLowerCase().indexOf(SearchString.toLowerCase()) != -1);
                });
                cmp.set("v.ExpenseList",bankStmtlist);  if(bankStmtlist.length<=0)  cmp.set("v.NoSlotsMessage",'No Data available');                     
    ...
    getRecord : function(component, event, helper){
        ...
        $A.createComponent("c:SupplierFieldSet",{
            RecordId : RecId, AccId :component.get("v.AccId"), ObName :"Expenses", PageLayoutName: "ERP7__Expense__c"
        },function(newCmp, status, errorMessage){
