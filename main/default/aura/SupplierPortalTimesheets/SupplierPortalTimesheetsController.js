({
	...
	doInit : function(component, event, helper) {
        ...
        helper.getTimeSheets(component,event);
	},
    ...
    searchUserRecord : function(cmp,event,helper){
        try{      
            ...
            if(SearchString!='' && SearchString!=undefined){   
                ...
                var bankStmtlist =[]; bankStmtlist=cmp.get("v.TSListDup");
                bankStmtlist = bankStmtlist.filter(function (el) {    
                    return ((el.Name).toString().toLowerCase().indexOf(SearchString.toLowerCase()) != -1);
                });
                cmp.set("v.TSList",bankStmtlist);  if(bankStmtlist.length<=0)  cmp.set("v.NoSlotsMessage",'No Data available');                     
     ...
     setShow : function(cmp,event,helper){
          ...
          cmp.set("v.Offset", 0);
          cmp.set("v.PageNum", 1);
          helper.getTimeSheets(cmp,event);
  ...
  getRecord : function(component, event, helper){
        ...
        $A.createComponent("c:SupplierFieldSet",{
            RecordId : RecId, AccId :component.get("v.AccId"), ObName :"Timesheets", PageLayoutName : "ERP7__Timesheet__c"
        },function(newCmp, status, errorMessage){
