                ...
                if(!PreventRecursiveLedgerEntry.testCasesTransactions){
                   ...
                   system.debug('invoice2upsert : '+invoice2upsert.size()); 
                    ...
                    if(invoice2upsert.size()>0){
                        ...
                        List<Sales_Order_Line_Item__c> SOLIS = [Select Id, ERP7__Sort_Order__c, ERP7__Detailed_Description__c, ERP7__Description__c, Total_Price__c,Name,ERP7__Invoice__c,ERP7__Invoiced_Quantity__c, ERP7__Sales_Order__c, ERP7__Sales_Order__r.ERP7__Issue_Invoice__c, ERP7__Quantity__c,ERP7__Base_Price__c,ERP7__Discount_Amount__c,ERP7__Other_Tax__c,ERP7__Product__c,ERP7__VAT_Amount__c, ERP7__Price_Product__c  From Sales_Order_Line_Item__c Where ERP7__Sales_Order__c IN: Trigger.newMap.keyset()];
                        ...
                        for(ERP7__Invoice__c inv :invoice2upsert){
                            ...
                            for(Integer i=0;i<SOLIS.size();i++){
                                ...
                                if(soli.ERP7__Sales_Order__r.ERP7__Issue_Invoice__c){
                                    ERP7__Invoice_Line_Item__c INLI = (soliMapToINLI.get(soli.ID) != null)? soliMapToINLI.get(soli.ID):new ERP7__Invoice_Line_Item__c(Invoice__c=inv.Id,Sales_Order_Line_Item__c = soli.Id, ERP7__Quantity__c=(soli.ERP7__Invoiced_Quantity__c >0.00)?soli.ERP7__Invoiced_Quantity__c:soli.ERP7__Quantity__c,ERP7__Base_Price__c =soli.ERP7__Base_Price__c, ERP7__Sub_Total__c=soli.ERP7__Base_Price__c, ERP7__Unit_Price__c = soli.ERP7__Price_Product__c,ERP7__Discount_Amount__c = soli.ERP7__Discount_Amount__c, ERP7__Other_Tax__c = soli.ERP7__Other_Tax__c, ERP7__Product__c = soli.ERP7__Product__c,ERP7__Total_Price__c = soli.Total_Price__c, ERP7__VAT_Amount__c = soli.ERP7__VAT_Amount__c, ERP7__Sort_Order__c = soli.ERP7__Sort_Order__c);
